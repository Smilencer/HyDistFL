...
import torch
from client.HyPeFL_ad import HyPeFLClient
from rich.progress import track
from tqdm import tqdm
from utils.args import get_HyPeFL_args
from utils.models_ad import HyperNetwork
from utils.util import get_dataloader, get_ad_data
from base_ad import ServerBase
...
class HyPeFLServer(ServerBase):
    def __init__(self):
        ...
        if os.listdir(self.temp_dir) != []:
            ...
            self.logger.log("Initializing clients model1...")
            # 初始化一个客户端模型参数列表。
        ...
        # +获取ad数据
        self.data, self.shared_loader, self.test_loader = get_ad_data(self.client_num_in_total, 0.5)
    ...
    def train(self) -> None:
        ...
        self.logger.log(selected_clients)
                ...
                with torch.no_grad():
                    for x, y in shared_data:
                        ...
                        if features.shape[0] == self.args.batch_size:
                            ...
                            padding_tensor = torch.zeros((self.args.batch_size - features.shape[0], 131072))
