...
class FedAVGManager(BasePSManager):
    def __init__(self, device, args):
        # 调用 BasePSManager 的初始化方法。
        super().__init__(device, args)
        # 设置每轮全局训练周期：从参数中获取每轮的全局训练周期 ？本地训练轮次
    ...
    def _setup_server(self):
        ...
        init_state_kargs = {} 
        # 启用了变分自编码器（VAE）模型，创建了一个 VAE 模型实例 VAE_model
        if self.args.VAE:
    ...
    def _setup_clients(self):
        ...
        for client_index in range(self.number_instantiated_client):
            ...
            # 为每个客户端创建了一个 FedAVGClient 实例，并将其加入到服务器端的客户端列表中。
            # 每个客户端负责本地的数据处理、模型训练和（可选的）VAE 训练
